...
export default function awsLambdaFastify (app, options = {}) {
  ...
  return function (event, context, callback) {
    ...
    let url = (options.pathParameterUsedAsPath && event.pathParameters && event.pathParameters[options.pathParameterUsedAsPath] && `/${event.pathParameters[options.pathParameterUsedAsPath]}`) || event.path || event.rawPath || '/'

    if (!options.retainStage && event.requestContext && event.requestContext.stage && event.requestContext.resourcePath && url.indexOf(`/${event.requestContext.stage}/`) === 0 && event.requestContext.resourcePath.indexOf(`/${event.requestContext.stage}/`) !== 0) {
      ...
      app.inject({ method, url, query, payload, headers, remoteAddress, payloadAsStream: options.payloadAsStream }, (err, res) => {
        ...
        if (err) {
          ...
          if (!options.payloadAsStream) {
            return resolve({ statusCode: 500, body: '', headers: {} })
          }
          const stream = res && res.stream()
          return resolve({
            meta: { statusCode: 500, headers: {} },
            stream: stream && stream.readableLength > 0 ? stream : Readable.from('')
          })
        }
